...
initPageAndLayout model =
    case Route.Path.fromUrl model.url of
        ...
        Route.Path.Explore ->
            let
                page : Page.Page Pages.Explore.Model Pages.Explore.Msg
                page =
                    Pages.Explore.page model.shared (Route.fromUrl () model.url)
            ...
            { page = 
                Tuple.mapBoth
                    Main.Pages.Model.Explore
                    (Effect.map Main.Pages.Msg.Explore >> fromPageEffect model)
                    ( pageModel, pageEffect )
            , layout = Nothing
            }

        Route.Path.Explore_Id_ params ->
            let
                page : Page.Page Pages.Explore.Id_.Model Pages.Explore.Id_.Msg
                page =
                    Pages.Explore.Id_.page model.shared (Route.fromUrl params model.url)
    ...
    case ( msg, model.page ) of
        ...
        ( Main.Pages.Msg.Explore pageMsg, Main.Pages.Model.Explore pageModel ) ->
            Tuple.mapBoth
                Main.Pages.Model.Explore
                (Effect.map Main.Pages.Msg.Explore >> fromPageEffect model)
                (Page.update (Pages.Explore.page model.shared (Route.fromUrl () model.url)) pageMsg pageModel)

        ( Main.Pages.Msg.Explore_Id_ pageMsg, Main.Pages.Model.Explore_Id_ params pageModel ) ->
            Tuple.mapBoth
                (Main.Pages.Model.Explore_Id_ params)
                (Effect.map Main.Pages.Msg.Explore_Id_ >> fromPageEffect model)
...
toLayoutFromPage model =
    case model.page of
        ...
        Main.Pages.Model.Explore pageModel ->
            Route.fromUrl () model.url
                |> Pages.Explore.page model.shared
                |> Page.layout pageModel
                |> Maybe.map (Layouts.map (Main.Pages.Msg.Explore >> Page))
    ...
    let
        ...
        subscriptionsFromPage =
            case model.page of
                ...
                Main.Pages.Model.Explore pageModel ->
                    Page.subscriptions (Pages.Explore.page model.shared (Route.fromUrl () model.url)) pageModel
                        |> Sub.map Main.Pages.Msg.Explore
                        |> Sub.map Page
...
viewPage model =
    case model.page of
        ...
        Main.Pages.Model.Explore pageModel ->
            Page.view (Pages.Explore.page model.shared (Route.fromUrl () model.url)) pageModel
                |> View.map Main.Pages.Msg.Explore
                |> View.map Page
...
toPageUrlHookCmd model routes =
    ...
    case model.page of
        ...
        Main.Pages.Model.Explore pageModel ->
            Page.toUrlMessages routes (Pages.Explore.page model.shared (Route.fromUrl () model.url)) 
                |> List.map Main.Pages.Msg.Explore
                |> List.map Page
                |> toCommands

        Main.Pages.Model.Explore_Id_ params pageModel ->
            Page.toUrlMessages routes (Pages.Explore.Id_.page model.shared (Route.fromUrl params model.url)) 
                |> List.map Main.Pages.Msg.Explore_Id_
                |> List.map Page
                |> toCommands
